...
@section('title', 'Edit {{ $resource_name }}')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Edit {{ $resource_name }}</h1>
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <form action="{{ route('{{ $resource_name_lower }}.update', ${{ $resource_name_lower }}->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    @switch($type)
                        @case('textarea')
                            ...
                            >{{ old($field, ${{ $resource_name_lower }}->{{ $field }}) }}</textarea>
                            @break
                            ...
                            >
                                ...
                                @foreach(${{ $field }}_options ?? [] as $option)
                                    <option value="{{ $option['value'] }}" 
                                            {{ (old($field, ${{ $resource_name_lower }}->{{ $field }}) == $option['value']) ? 'selected' : '' }}>
                                        {{ $option['label'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @case('checkbox')
                            <div class="flex items-center">
                                <input 
                                    ...
                                    {{ (old($field, ${{ $resource_name_lower }}->{{ $field }}) ? 'checked' : '') }}
                                >
                        ...
                        @case('file')
                            <input 
                                ...
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error($field) border-red-500 @enderror"
                            >
                            @if(${{ $resource_name_lower }}->{{ $field }})
                                <p class="mt-1 text-sm text-gray-500">Current: {{ ${{ $resource_name_lower }}->{{ $field }} }}</p>
                            @endif
                            @break
                        ...
                        @default
                            <input 
                                ...
                                id="{{ $field }}" 
                                name="{{ $field }}" 
                                value="{{ old($field, ${{ $resource_name_lower }}->{{ $field }}) }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error($field) border-red-500 @enderror"
                                placeholder="Enter {{ $field }}"
                            >
                    @endswitch
